...
public static class EntityTypeExtensions
{
	extension(EntityType entityType)
	{
		...
		private bool IsWeakPoint(int userData)
		{
			return entityType switch
			{
		...
		public int GetInitialTransmuteHp()
		{
			return entityType switch
			{
		...
		public int GetDamage(EntityType daggerType, int userData)
		{
			if (!entityType.IsEnemy())
				throw new InvalidOperationException($"Type '{entityType}' must be an enemy.");

			if (!daggerType.IsDagger())
				throw new InvalidOperationException($"Type '{daggerType}' must be a dagger.");

			if (!entityType.IsWeakPoint(userData))
				return 0;
			...
			return daggerType switch
			{
				...
				EntityType.Level3HomingDagger => entityType switch
				{
				...
				EntityType.Level4HomingDagger => entityType switch
				{
					EntityType.Leviathan => 1, // Homing deals normal damage to Leviathan (and Orb, which is the same enemy in this context)
				...
				EntityType.Level4HomingSplash => entityType switch
				{
